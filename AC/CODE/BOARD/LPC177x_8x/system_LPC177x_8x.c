...
{
#if (CLOCK_SETUP)                       /* Clock Setup                        */   // Ñ¡ÔñÍâ²¿¾§Õñ
  LPC_SC->SCS       = SCS_Val;
  if (SCS_Val & (1 << 5)) {             /* If Main Oscillator is enabled      */
    while ((LPC_SC->SCS & (1<<6)) == 0);/* Wait for Oscillator to be ready    */
  }

  LPC_SC->CLKSRCSEL = CLKSRCSEL_Val;    /* Select Clock Source for sysclk/PLL0 Ñ¡ÔñÍâ²¿¾§Õñ×÷ÎªsysclkºÍ£Ð£Ì£Ì£°ÎªÔ´Í· */

#if (PLL0_SETUP)
  LPC_SC->PLL0CFG   = PLL0CFG_Val;       // m=9  p=0  divi 1
...
#if (PLL1_SETUP)
  LPC_SC->PLL1CFG   = PLL1CFG_Val;      // m=3 p=2 dive 4
  ...
  LPC_SC->CCLKSEL   = CCLKSEL_Val;      /* Setup Clock Divider                */   //Ñ¡Ôñ£Ð£Ì£Ì¸ø£Ã£Ð£Õ¹©£¬ÎÞ·ÖÆµ
  LPC_SC->USBCLKSEL = USBCLKSEL_Val;    /* Setup USB Clock Divider            */
  LPC_SC->EMCCLKSEL = EMCCLKSEL_Val;    /* EMC Clock Selection                */
  LPC_SC->PCLKSEL   = PCLKSEL_Val;      /* Peripheral Clock Selection         //¡¡ÍâÉè¡¡2·ÖÆµ */
...
#ifdef 	__FLASH_MODE__
  SCB->VTOR  = 0x00000000 & 0x3FFFFF80;
#endif

#ifdef  __LOW_ADDR
  SCB->VTOR  = 0x00010000 & 0x3FFFFF80;
#endif

#ifdef  __HIGH_ADDR
  SCB->VTOR  = 0x00048000 & 0x3FFFFF80;
#endif
}
